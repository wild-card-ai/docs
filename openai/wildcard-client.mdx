...
```python
from wildcard_core.errors import (
...
try:
    result = await client.run_tools(completion)
except AuthenticationError as e:
    # Handle authentication issues
    print(f"Auth failed: {e.message}")
    if e.auth_info:
        print(f"Required scopes: {e.auth_info.required_scopes}")
    ...
    print(f"Invalid args: {e.validation_errors}")
```
