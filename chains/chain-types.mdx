---
title: 'Chain Types'
description: 'Different types of chains supported in Wildcard'
---

Wildcard supports several types of chains to handle different workflow patterns. Each type is optimized for specific use cases.
...
async def project_workflow():
    ...
    issue = await client.run_tool_with_args(
        tool_name=Action.GitHub.ISSUES_CREATE,
        title="New Feature Request",
        body="Implementation needed"
    )
    ...
    await client.run_tool_with_args(
        ...
        channel="dev-team",
        text=f"New issue created: {issue.processed_response.data['html_url']}"
    )
    
    # Step 3: Schedule follow-up
    await client.run_tool_with_args(
        tool_name=Action.GoogleCalendar.EVENTS_INSERT,
        summary="Review New Feature Request",
        description=f"Discuss issue: {issue.processed_response.data['html_url']}"
    )
```
